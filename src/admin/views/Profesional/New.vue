<template lang="pug">
  .new
    Notification(v-if="notification.show", :type="notification.type", @cancel="closeNotification()", @save="submitForm()", @finish="goToProfesional()")
                ...
                .card.profesional.has-background-grey-lighter
                  ...
                  .card-image.px-6.pb-3
                    span(@mouseover="hovered = true", @mouseout="hovered = false")
                      figure.image.is-square.has-background-white(v-show="!hovered")
        ...
        .column
          .columns.px-6.buttons
            .column.is-one-third.is-flex.is-justify-content-flex-start
              button.button.is-rounded.is-medium.is-white.is-danger.is-inverted(disabled)
                span.icon
                  i
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goToProfesional() {
        this.$router.push(`/profesionales/${this.tmpId}`);
      },
      saveProfesional() {
        this.sent = true;
        const self = this;
            ...
            .then(
              ({ status, data }) => {
                if (status === 201) {
                  ...
                  //self.$router.replace(`/profesionales/${data._id}`);
                }
              },
              error => {
                console.error(error);
