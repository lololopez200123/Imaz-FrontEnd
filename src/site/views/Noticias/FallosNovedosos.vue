<template lang="pug">
  .fallosnovedosos
    Header(
      seccionActiva="fallos-novedosos")
    .black-screen.fadeIn(
      v-onEscClose=""
      @click="closeFalloActiva"
    ...
    .filtros
      ...
      .filtro.expediente
        input(
          type="text"
          v-model="expedienteActivo"
          placeholder="Nro. Expte")
      .filtro.caratula
        input(
          type="text"
          v-model="caratulaActivo"
          placeholder="Carátula")
    .lista-fallosnovedosos
      .lista-anio(
        v-swipe.setScrollPrev.setScrollNext="anio"
        v-for="(fallos, anio, index) in fallosnovedosos"
        :key="anio"
        ...
        .lista-flechas
          ...
          .contenedor-lista(:class="'scroll-' + scroll[anio]")
            .lista
              .fallo(
                v-for="(fallo, index) in byCaratulaAndExpediente(fallos)"
                :key="fallo._id"
                :class="['mod-' + index % 3]")
                .box(
                  @click="setFalloActiva(anio, index)")
                  .caratula
                    span {{fallo.caratula}}
                  .autor
                    span Expte. nro {{fallo.expediente}}
              ...
              .fallo(
                ...
                .cabecera
                  ...
                  .expediente-pubdate
                    ...
                    .pubdate
                      span {{meses[new Date(fallo.pubdate).getMonth()]}}, {{new Date(fallo.pubdate).getFullYear()}}
                .cuerpo
                  .texto(v-html="fallo.texto")
...
export default {
  ...
  directives: {
    onEscClose(el, bind, vnode) {
      const closePub = ev => {
        if (ev.which === 27) {
          vnode.context.closeFalloActiva();
  ...
  mounted() {
    this.$http({
      ...
      ({ data }) => {
        this.fallosnovedosos = data.reduce((fallosnovedosos, p) => {
          if (!fallosnovedosos[`-${moment(p.pubdate).year()}`]) {
            fallosnovedosos[`-${moment(p.pubdate).year()}`] = [];
            this.anios.push(moment(p.pubdate).year());
            this.scroll[`-${moment(p.pubdate).year()}`] = 0;
          }
          fallosnovedosos[`-${moment(p.pubdate).year()}`].push(p);
          return fallosnovedosos;
        }, {});
      },
      error => {
        console.error(error);
  ...
  methods: {
    ...
    //   this.mesActivo = mes;
    // },
    byCaratulaAndExpediente(arr) {
      let outArr = [];
      if (this.expedienteActivo && this.caratulaActivo) {
        outArr = arr.filter(
          fallo =>
            this.expedienteActivo === fallo.expediente &&
            RegExp(this.caratulaActivo, "gi").test(fallo.caratula)
        );
      } else if (this.expedienteActivo) {
        outArr = arr.filter(fallo => this.expedienteActivo === fallo.expediente);
      } else if (this.caratulaActivo) {
        outArr = arr.filter(fallo => RegExp(this.caratulaActivo, "gi").test(fallo.caratula));
      } else {
        outArr = arr;
      }
      return outArr;
    ...
    setScrollNext(anio) {
      if ((this.scroll[anio] + 1) * 3 < this.fallosnovedosos[anio].length) {
        this.scroll = { ...this.scroll, [anio]: this.scroll[anio] + 1 };
      }
    },
    setScrollShowNext(anio) {
      if (this.publicionAcitva.index + 1 < this.fallosnovedosos[anio].length) {
        this.publicionAcitva = { anio, index: this.publicionAcitva.index + 1 };
      }
    },
    setFalloActiva(anio, index) {
          ...
          .lista {
            ...
            .fallo {
              ...
              .box {
                ...
                //   background-image: url('../../../assets/images/Home/fondo-laley.jpg');
                //   z-index: 1;
                //   opacity: 0.7;
                // }
                &::after {
